...
目前，通用的单钥加密算法为AES（Advanced Encryption Standard），通用的双钥加密算法为RSA（ Rivest-Shamir-Adleman）。
...
CSR 是 Cerificate Signing Request 的英文缩写，即证书请求文件，也就是**证书申请者在申请数字证书时由 CSP (加密服务提供者)在生成私钥的同时也生成证书请求文件（私钥和 CSR 同时生成）**，证书申请者只要把 CSR 文件提交给证书颁发机构后，证书颁发机构使用其根证书私钥签名就生成了证书公钥文件，也就是颁发给用户的证书。通过如下方式申请：
...
这种带括号某某人的 MacBook 的证书怎么产生的呢？这是因为我们在 Xcode 中登录了开发者账号。Xcode 自动帮你在签名的时候生成了一个私钥以及证书。

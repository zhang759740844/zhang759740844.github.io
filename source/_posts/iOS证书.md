...
目前，通用的**单钥加密算法为AES**（Advanced Encryption Standard），通用的**双钥加密算法为RSA**（ Rivest-Shamir-Adleman）。
...
不知道你有没有注意到，个人证书（不论是使用开发证书还是发布证书）的 app 安装的时候，都需要联网验证证书（如果你用 Xcode 安装，那么电脑联网就行了，如果电脑没联网，那么手机必须联网），并且企业证书要求更加苛刻，每次打开都需要验证证书。那么为什么要验证证书呢？

因为开发者可能因为各种原因删除之前的证书，表示该证书所对应的公钥私钥不再可信。比如某人原来管着发布证书，但是这个人离职了，那么防止他用原证书上架恶意 app，就要把之前的这个证书删掉。这样在提交 app 的时候就会被拒了。即使在删除之前已经提交了，也会因为有一个审核期，不会影响任何用户。但是企业证书因为是即刻上线，别人发布新的恶意 app，立刻就会有人安装 。所以可能别人刚离职，就发布了一个恶意应用，并且有人已经下载了。为了防止这种情况导致的影响，所以企业证书每次打开 app 都需要联网验证证书，如果证书不存在则不能打开。

...
虽然我很矿局 wxs，但是由于小程序不支持 computed，由于代码的美观，少在 data 中放几个字段，就需要借助 wxs 在 wxml 中生成计算后的数据。
...
实现的方式是给要设置悬停的视图设置 id，这是一个占位视图。然后在小程序滚动方法中获取该视图，判断该视图的位置，控制悬停视图的显示和隐藏：
...
上面的代码中，将占位视图的 id 设置为 `select` ，通过上面的代码获取到了该元素。当滚动后 top 小于 0，即滚动到屏幕外，并且悬停视图没有展示的时候，设置状态 `hideBeginSelect` 为 false，反之设为 true。

在视图中通过这个状态控制 `display`

```html
<view style="display: {{hideBeginSelect ? 'none' : 'flex'}}">
```
...
很多时候我们需要根据组件传入的值，来设置组件使用不同的 class。我们可以使用或或者三元选择符，但是需要使用 `{{}}` 包裹，只要返回一个 class 的名字即可：
...
很多时候，我们需要传入值来控制 style，传入的值需要使用 `{{}}` 包裹：

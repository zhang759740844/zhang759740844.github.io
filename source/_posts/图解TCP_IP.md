...
- **4-7层交换机**：负责处理OSI模型中从传输层到应用层的数据。例如实现负载均衡
- **网关**：**负责OSI传输层到应用层的数据进行转换和转发**的设备。

![](https://github.com/zhang759740844/MyImgs/blob/master/MyBlog/wangguan_1.png?raw=true)
...
数据链路层是模型中的第2层，该层对接受到物理层传输过来的比特流进行分组，一组电信号构成的数据包，就叫做"帧"，**数据链链路层就是来传输以"帧"为单位的数据包，把数据传递给上一层（网络层）**，帧数据由两部分组成：帧头和帧数据，帧头包括接受方**物理地址（**就是下一级的网卡地址**）**和其他的网络信息，帧数据就是要传输的数据体。数据帧的最长为1500字节，如果数据很长，就必须分割成多个帧进行发送。
...
路由器之间可以互换目标网络的方向及其距离的相关信息(像是一个动态规划，A-D 的最短距离等于 A-B1-D 或者 A-B2-D 之中的较短者，所以需要计算 B1-D，B2-D 的最短距离)。最后，路由器中会生成一个某段IP对应某个路由的表。

缺点是不太容易判断别的路由器的信息是否正确。

#### 链路状态算法

链路状态算法保存网络中的所有路由器及网络的拓扑图。比较费时费性能。
...
传输层的 TCP UDP 用来识别所传输的数据最终发给哪个引用程序，使用端口号这一识别码。

...
这是一个各个组件相互调用的例子。同上面所说的一样，`Component` 中有抽象中介者 `mediator`，会在生成具体同事类(即各个组件)的时候，`setMediator()` 设置具体中介者 `ConcreteMediator`。它提供一个 `componentChanged()` 方法供具体同事类调用，这个方法用来处理跳转啊交互之类的逻辑。

...
在 ES6 之前，没有类的概念，只有一个个函数，通过 `prototype`实现。

`prototype` 是函数的属性，这个属性是指向一个对象的引用，这个对象称为**原型对象**，原型对象包含函数实例共享的方法和属性，也就是说将函数用作构造函数调用（使用new操作符调用）的时候，新创建的对象会从原型对象上继承属性和方法。
...
![](https://github.com/zhang759740844/MyImgs/blob/master/MyBlog/js_base_1.png?raw=true)
...
ES6模块的运行机制与CommonJS不一样，**它遇到模块加载命令`import`时，直接执行相应的文件**
